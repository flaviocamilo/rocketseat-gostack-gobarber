...
class UserController {
	async store(request, response) {
		...
		if (!(await schema.isValid(request.body))) return response.status(400).json({ error: 'Validation failed.' });
	...
	async update(request, response) {
		...
		if (email && email !== user.email) {
			...
			if (userExists) return response.status(401).json({ error: 'User already exists.' });
		}
